 ...
 *
 * PDF extraction, pursuit management, result validation, HTML review generation.
 */
...
class TestResult {
  ...
  check(condition, message) {
    this.checks.push({ pass: !!condition, message });
  ...
  recordUsage(usage, model = 'haiku') {
    const ti = usage?.input_tokens || 0;
    const to = usage?.output_tokens || 0;
    this.tokensIn += ti;
    this.tokensOut += to;
    if (model === 'haiku' || model?.includes('haiku')) {
  ...
  const sectionHTML = sections.map(s => {
    ...
    if (s.type === 'table') {
      ...
      content = `<div class="cards">${s.items.map(i =>
        `<div class="card"><div class="card-label">${i.label}</div><div class="card-value">${i.value}</div>${i.detail ? `<div class="card-detail">${i.detail}</div>` : ''}</div>`
      ).join('')}</div>`;
    } else if (s.type === 'text') {
      content = `<div class="text-block">${s.content}</div>`;
    } else if (s.type === 'json') {
      content = `<pre class="json-block">${JSON.stringify(s.data, null, 2)}</pre>`;
    } else if (s.type === 'list') {
      content = `<div class="action-list">${s.items.map(i =>
        `<div class="action-item"><div class="action-priority">${i.priority || ''}</div>${i.text}</div>`
      ).join('')}</div>`;
    }
    return `<div class="section"><h2>${s.title}</h2>${s.subtitle ? `<div class="section-sub">${s.subtitle}</div>` : ''}${content}</div>`;
  }).join('\n');
...
.header .sub{color:#60F5F7;font-size:10px;letter-spacing:.08em;text-transform:uppercase;margin-top:2px;}
...
th{background:#5CA3B6;color:#fff;font-size:10px;letter-spacing:.04em;text-transform:uppercase;padding:8px 12px;text-align:left;white-space:nowrap;}
...
.card-value{font-size:24px;font-weight:700;color:#5CA3B6;}
.card-detail{font-size:11px;color:#576482;margin-top:4px;}
...
.badge{display:inline-block;padding:1px 6px;font-size:9px;letter-spacing:.04em;text-transform:uppercase;}
.badge-pass{background:rgba(45,138,94,.1);color:#2D8A5E;}
.badge-fail{background:rgba(193,80,80,.1);color:#C15050;}
.badge-warn{background:rgba(198,138,29,.1);color:#C68A1D;}
</style>
</head>
<body>
<div class="header">

...
| Bid Execution | Full bid lifecycle management. 82 activities, 10 workstreams, governance gates, submissions engine | Architecture complete — build in progress |
...
- **82 activities** across **10 workstreams**
...
**Context window constraint:** For a bid with 82 activities, Claude must never load all records simultaneously. The MCP server must expose filtered queries. The timeline analysis skill must use `get_activities_due_within()` and `get_critical_path()` rather than loading everything.

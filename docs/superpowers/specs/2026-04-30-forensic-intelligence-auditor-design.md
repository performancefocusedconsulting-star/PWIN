...
A platform-level reference table the auditor uses across all asset profiles. The producing skills emit `sourceTier` on each claim; the auditor uses it for evidence integrity, contradiction handling, and decision-safety severity weighting.
...
- `claimId` (from the producing skill's structured claims block — see §11)
- `claimText`
...
- `claimId` — stable, traceable identifier so the narrative can cite the claim and the audit log can refer to it
- `claimText` — the assertion itself, in plain prose
- `claimDate` — when the producing skill asserted this (the date of the audit-relevant assertion, not the date of the dossier as a whole)
- `source` — URL, document reference, or structured identifier for where the claim comes from
- `sourceDate` — when the source itself was published or last updated
- `sourceTier` — 1, 2, 3, or 4, from the table in §9

All field names use camelCase, matching the rest of the dossier shape (`sourceRegister.sources[].sourceId`, `publicationDate`, etc.). The narrative renders the claims and cites them by `claimId`. The auditor only audits the `claims[]` block.

**Integrator extension.** Integrator skills (incumbency-advantage-displacement-strategy in V1) may carry an optional `derivedFrom` field on each claim — an array of upstream claim IDs the integrator synthesised the claim from, in the format `["BUYER:CLM-014", "SUPPLIER:CLM-022"]`. This lets the auditor walk the chain back to source-level evidence through skill boundaries. Documented in the canonical contract at `pwin-platform/skills/agent2-market-competitive/master/CLAIMS-BLOCK-SCHEMA.md`.

### 11.2 V1.1 contract — upstream volatility tag

V1.1 adds an additional optional field per claim:
...
V1.0 does not require this field. Volatility in V1.0 is profile-derived from the section the claim sits in. V1.1 lets a claim override its section's profile-default. `volatility` is independent of the integrator's `derivedFrom` extension — a V1.1 integrator claim may carry both, neither, or just one.
...
- The claim has no `claimDate` or `sourceDate` in the V1.0 contract sense — Source Traceability and Freshness both score low
